...
`email`,
`password
`, `birthday`, `gender`, `contact`, `nid`, `address`, `dept`, `degree`, `pic`) VALUES
...
pid`,
`eid
`, `pname`, `duedate`, `subdate`, `mark`, `status`) VALUES
...
--
ALTER TABLE `project`
ADD CONSTRAINT `project_ibfk_1` FOREIGN KEY
...
--
ALTER TABLE `rank`
ADD CONSTRAINT `rank_ibfk_1` FOREIGN KEY
(`eid`) REFERENCES `employee`
(`id`) ON
DELETE CASCADE ON
UPDATE CASCADE;
